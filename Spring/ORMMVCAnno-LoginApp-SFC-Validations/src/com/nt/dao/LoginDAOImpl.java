...
public class LoginDAOImpl implements LoginDAO {
	// private static final String AUTHENTICATE_QUERY="SELECT COUNT(*) FROM
	// UserHLO WHERE UNAME=:un AND PWD=:pass";
	@Resource
	...
	public long validate(UserHLO bo) {
		long count = 0;
		List<?> list = null;
		list = ht.findByNamedQueryAndNamedParam("AUTH_QUERY", new String[] { "un", "pass" },
				new String[] { bo.getUname(), bo.getPwd() });

		count = (long) list.get(0);
		/*
		 * //bulkupdate int
		 * cnt=ht.bulkUpdate("delete from UserHLO where uname=?","raja");
		 * System.out.println("records effected:"+cnt); return count;
		 */
